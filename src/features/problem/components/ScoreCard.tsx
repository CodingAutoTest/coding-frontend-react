...
export const ScoreCard: React.FC<ScoreCardProps> = ({ label, score, tooltip }) => (
  <div className="flex items-center justify-between bg-white p-5 rounded-2xl shadow-md relative">
    <div className="flex items-center gap-4">
      <img src={IMAGES.PROBLEM_RESULT.GRAPH} alt="graph" className="w-10 h-10" draggable={false} />
      <div className="flex flex-col">
        <span className="text-sm text-gray-500">{label}</span>
        <span className="text-xl font-inter font-bold text-DEFAULT">{score ?? '???'}점</span>
      </div>
    </div>
    {/* info 툴팁 */}
    <div className="relative group cursor-pointer">
      <img src={IMAGES.PROBLEM_RESULT.INFO} alt="info" className="w-5 h-5" draggable={false} />
